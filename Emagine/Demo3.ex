...
sequence squares            squares = repeat({0,0,0,0,0},10)
...
integer x_pos               x_pos = 1
integer y_pos               y_pos = 1
integer numx                numx = 1
integer numy                numy = 1
...
while get_key() != 13 do
	...
	x_pos = x_pos + numx
	y_pos = y_pos + numy
	if x_pos >= x2-length(squares[counter][5][1])-2 then
	    numx = -1
	    if rand(2) = 1 then
		numy = -1
	    end if
	...
	if y_pos >= y2 - length(squares[counter][5])-2 then
	    numy = -1
	    if rand(2) = 1 then
		numx = -1
	    end if
    ...
    draw_display()
    --clear_display(0)  --Uncomment this for a different result!
end while
